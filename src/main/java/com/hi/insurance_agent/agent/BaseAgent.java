 ...
 */
@Slf4j
@Data // @Data是 lombok 提供的注解，它的作用是自动生成类的常用方法，从而减少样板代码。
public abstract class BaseAgent {
    ...
    public String run(String userPrompt) {
        if (agentState != AgentState.IDLE) {
            throw new RuntimeException("Cannot run agent from state: " + agentState);
        }
        if (StrUtil.isBlank(userPrompt)) {
            throw new RuntimeException("Cannot run agent with empty user prompt");
        }
        // 基本输入清洗，避免因换行/空白导致的不一致
        ...
        try {
            for (int i = 0; i < this.maxSteps && this.agentState != AgentState.FINISHED; i++) {
                int stepNumber = i + 1;
